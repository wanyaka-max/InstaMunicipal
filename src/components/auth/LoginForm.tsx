...
const LoginForm = ({ onSuccess }: LoginFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.endsWith(".gov")) {
      setError(
        "Only government email addresses are allowed for this login. Regular users should register for an account.",
      );
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white/90 backdrop-blur-md rounded-xl shadow-md overflow-hidden p-6 space-y-6 border border-gray-100">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-900">Government Login</h2>
      ...
      <div className="text-center text-sm">
        <span className="text-gray-600">Don't have a government account?</span>{" "}
        <Link
